...
| `ballot`          | Ballot Builder | Availability panel for the [Ballot Builder](#ballot-builder). Until the open date it says when to come back; the date comes from `GET /api/ballot/availability`, never from the page. Becomes the real builder UI in phase 5. |
...
`showView('ballot')` triggers `loadBallotAvailability()`.
...
| GET    | `/api/ballot/availability`        | **Public, no auth.** `{open, availableFrom, message}` — when the Ballot Builder opens. Unauthenticated on purpose: the panel has to tell a signed-out visitor when to come back, and making them sign in to be told "not yet" is a poor trade. |
| POST   | `/api/ballot/lookup`              | **Signed-in only. Costs 3 credits (the flat ballot price).** Resolves the offices + candidates on a voter's ballot. Body: `{state, city?, county?, address?, electionDate?}`. **v1 scope is enforced server-side: California, Texas, Louisiana only, candidates only** (400 `state_unsupported` otherwise). Prompt assembled server-side from `buildBallotRequest`; web_search, no adaptive thinking. The address is used for the lookup and **never stored or logged** — the ledger ref is `ballot:<STATE>`. 502 for any upstream failure. See [Ballot Builder](#ballot-builder). |
...
- **Only a schema-valid audit is cached** (`validateAudit().ok`). Otherwise one malformed response is served to every later caller for the rest of the TTL. The caller still gets their audit either way.
...
**The cache announces itself at boot** — `Audit cache: enabled (ttl 7d, prompt <hash>)`, or a shouty `DISABLED` line. This is not decoration. A dormant cache is invisible from the outside: every audit simply runs and bills, exactly as it did before the cache existed, so "switched itself off" and "working perfectly" produce identical logs, identical responses, and an identical Anthropic bill. The first version of this code gated on `PROJECT_ID` — a variable Cloud Run does not set — and therefore shipped switched off through a green 243-test suite, a passing CodeQL gate, and a successful deploy. It was caught only by running a real audit on dev and finding the credit debited and the collection empty. **Never gate the cache on an env var that is absent in production, and never let it disable itself quietly.** Regression test: "the live cache wiring is enabled without GOOGLE_CLOUD_PROJECT" in `analyze.integration.test.js`.
...
A voter enters a location, the app resolves what is on their ballot, audits each candidate against the Integrity Index, and hands back a printable scorecard. Built in phases; **phases 1–2 are in, 3–6 are not**, so nothing about ballots is user-visible yet.

| Phase | What | Status |
|------:|------|--------|
| 1 | [Shared audit cache](#shared-audit-cache) | **done** |
| 2 | Ballot lookup: Census districts + `POST /api/ballot/lookup`, date-gated | **done, unvalidated** |
| 3 | `ballots` collection + per-candidate entitlement | not started |
| 4 | Client-side fan-out (3 concurrent) + progressive render | not started |
| 5 | Ballot view (7th nav tab) + ballot PDF | not started |
| 6 | Tests + ops | not started |

**Settled product decisions.** Ballots are populated by Claude web_search (no vendor contract); the price is **flat — 3 credits per ballot regardless of race count**, charged once at lookup, which only works because the shared cache makes the per-candidate audits nearly free; output is a **printable PDF**, with no public share link and no pick-marking in v1; the whole thing is **signed-in and metered**, like every other route that spends the Anthropic key.

**Districts come from the Census, not the model.** `backend/lib/districts.js` resolves the address against the **US Census geocoder** (`geocoding.geo.census.gov`) before the model is called. This is not an optimization, it is the difference between working and not: determining which districts an address sits in is geocode-then-point-in-polygon, and a traced attempt to make the model do it burned its search budget in 18 seconds and then ground through **two and a half minutes of code-execution rounds without emitting a single character**. The geocoder answers in ~0.3s, exactly. Two consequences worth keeping: **the street address never reaches Anthropic** (only district names do), and district accuracy stops being a model inference. Note the geocoder needs **no API key** — that is `api.census.gov`, the separate Census *Data* API. It covers federal, state-legislative and county only; **municipal districts (city council, school board, judicial subdistricts) are not in Census geography**, so those races stay in `unresolved` rather than being guessed. Geography keys are matched by substring because they carry the Congress number and redistricting year (`119th Congressional Districts`), which change underneath us.

**The builder is date-gated, and that is a data fact rather than a product preference.** Before candidate filing closes there is no authoritative list of who is running, and asking anyway does not return an empty result — it returns *nothing*, for minutes, until something times out. The identical query against a settled past election answers correctly in ~24 seconds (verified: LA CD-2, November 2024, including the jungle-primary structure). So `backend/lib/ballotWindow.js` refuses before the open date (`BALLOT_AVAILABLE_FROM`, default **2026-10-08**) as the *first* statement in the handler — no credit, no model call, not even a geocoder request — and `GET /api/ballot/availability` lets the UI say when to come back without making a failing request first.

> **The displayed date is read from the configured date string, never by re-formatting the parsed instant in a timezone.** Those disagree: `2099-01-01T00:00:00-05:00` rendered in `America/Chicago` reads "December 31", because midnight at −05:00 is the previous evening in Central. The shipped October default hides the bug (October is also −05:00), so it would have surfaced only when someone changed the date — and then every visitor would have been told to come back a day early. Regression test: "the displayed date is the configured calendar date, not a re-zoned one".

**Why the scope is this narrow.** v1 covers **California, Texas, and Louisiana** and **candidates only** — no measures, propositions, or bond issues. A ballot this tool gets wrong sends someone to the polls with bad information, so the bound is set to what can be hand-verified against real sample ballots, not to what the model will cheerfully answer for anywhere. The state allowlist lives in `prompts.BALLOT_STATES` and is enforced in the route before anything is billed. Widening it means hand-verifying the new state first.

**The address is the sensitive part.** A street address is needed to resolve legislative and municipal districts, and it is the most sensitive thing this app ever receives. It is passed to the model and then dropped: **never written to Firestore, never written to a log line**, and the `credit_ledger` ref is `ballot:<STATE>` rather than the subject. The ledger is append-only and long-lived, so an address landing there would be permanent. Regression tests: "the street address never reaches the credit ledger" and "the address is used for the lookup even though it is never stored".

**Still unvalidated against a real upcoming ballot.** Every test uses a mocked Anthropic client, and no live lookup has ever returned a ballot for an election that has not happened yet — because none could, until filing closes. The first real validation is the day the window opens. Until then the route is correct, guarded, and unproven.

**Accuracy posture.** Claude-sourced ballots will be wrong on some down-ballot races. The prompt tells the model to prefer official sources (Secretary of State, county elections office, candidate filings), to omit anything it cannot verify rather than invent it, and to report what it could not resolve in `unresolved`. `validateBallot()` checks the shape; a race with no candidates and a `confidence` figure are both signals the UI will need to surface. When phase 5 lands, the PDF must carry the lookup date, a link to the official sample ballot, and language that these are scores rather than endorsements.
...
| `GOOGLE_CLOUD_PROJECT` / `GCLOUD_PROJECT` | no | Passed to the Firestore client. **Cloud Run does NOT set this** — that is App Engine / Cloud Functions behaviour; Cloud Run serves the project id from the metadata server, which the Firestore client reads on its own. So it is normally **unset in production**, and anything that treats it as "am I deployed?" is reading a variable that is empty on every deployed instance. It cost a silently-dormant audit cache once already — see [Shared audit cache](#shared-audit-cache). |
...
| `BALLOT_AVAILABLE_FROM` | no      | ISO 8601 date-time (with offset) when the Ballot Builder opens. Defaults to `2026-10-08T00:00:00-05:00`. A malformed value logs a `CONFIG ERROR` and falls back to the default rather than opening or closing the gate forever. **Forwarded by the pipeline from the optional repo/Environment variable of the same name — set it there, not on the Cloud Run service.** A value set directly on the service works until the next deploy, which rewrites the whole env var set and silently reverts it. See [Ballot Builder](#ballot-builder). |
| `AUDIT_CACHE_TTL_DAYS`  | no      | How long a cached AI audit stays servable. Defaults to `7`. Deliberately short — this is a tool people use to decide a vote, and a week-old read of a campaigning candidate is already at the edge of useful. It costs more than a longer window (the same subject is re-audited weekly, not monthly); that is the intended trade, freshness over margin. **`0` disables the cache entirely** — a kill switch that needs no code change. |
...
| *(optional)* `ANTHROPIC_MODEL`, `REGISTRATION_EMAIL`, `REGISTRATION_FROM`, `DONATION_URL`, `ALLOWED_ORIGIN`, `FREE_CREDITS`, `MAX_CREDIT_GRANT`, `APP_URL`, `BALLOT_AVAILABLE_FROM`, `APP_VERSION` | Passed through as env vars only when set; unset → the app's built-in defaults apply. |
